...
var (
	...
	testCases = []struct {
	...
	}{
		{
			...
			map[string]string{
				"rootFolder":        "../",
				"exampleFolderPath": "examples/eks-example-cluster"},
		},
	}

	// apply the infrastructure, same as running `terraform apply`
	applyModules = []string{
		"full_apply",
	}
	...
	// to be destroyed.  This ensures resources are destroyed in the right order
	// so there are no errors on destroy
	destroyModules = []string{
		"module.addons",
		"module.eks_cluster",
		"module.vpc",
		"full_destroy", //destroy everything else that was not destroyed by living in a module, like independent resources & data blocks.
	}

	/*Update the expected Output variables and values*/
	outputParameters = [...]Outputs{
	{"eks_cluster_id", "eks-example-cluster", "equal"},
