...
resource "kubernetes_secret" "argocd_gitops" {
  ...
  data = {
    username = jsondecode(data.aws_secretsmanager_secret_version.https_credential_secret_version[each.key].secret_string)["USERNAME"]
    password = jsondecode(data.aws_secretsmanager_secret_version.https_credential_secret_version[each.key].secret_string)["PASSWORD"]
    type     = "git"
