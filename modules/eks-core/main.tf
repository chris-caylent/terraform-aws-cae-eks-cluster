...
resource "aws_eks_cluster" "this" {
  ...
  dynamic "encryption_config" {
    ...
    content {
      provider {
        key_arn = var.create_kms_key ? var.kms_key_arn : encryption_config.value.provider_key_arn
      }
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = var.create_kms_key ? [var.kms_key_arn] : [for config in var.cluster_encryption_config : config.provider_key_arn]
      },
    ]
  })
